...
export default {
    ...
    methods: {
        ...
        getResults(page = 1) {
            axios.get("/api/manager?page=" + page)
            .then(response => {
        ...
        loadUsers() {
            ...
            axios.get("/api/manager", { params: this.filterForm })
            .then(({ data }) => {
        ...
        updateUser() {
            this.form.put("/api/manager/" + this.form.id)

            .then(() => {
                $("#addNewUser").modal("hide");
                Swal.fire("Updated!", "Store Manager Updated Successfully.", "success");
                this.show_password = 1;
                this.loadUsers();
                this.getUser();
            })
        ...
        deleteUser(id) {
            Swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    this.form.delete("/api/manager/" + id)
                    .then(() => {
                        Swal.fire(
                            "Deleted!",
